...
@layer_register()
def QuantizedActiv(x, nbit=2, update_basis=True):
    """
    Quantize activation.
    Args:
    ...
    * ``basis``: basis of quantized activation.
    ...
    with tf.variable_scope('ActivationQuantization'):
        ...
        if ctx.is_main_training_tower:
            ...
            # eye = tf.eye(BTxB.get_shape().as_list()[0]) * 0.0001
            # BTxB_inv = tf.matrix_inverse(BTxB + eye)
            ...
            if update_basis:
                ...
                for i in range(nbit):
                    tf.summary.scalar('new_basis%d' % i, new_basis[i][0])

            for i in range(nbit):
                tf.summary.scalar('basis%d' % i, basis[i][0])
    ...
    with tf.variable_scope(name):
        ...
        if ctx.is_main_training_tower:
            ...
            if nbit > 2:
                ...
                # eye = tf.eye(BTxB_transpose.get_shape().as_list()[0]) * 0.0001
                # BTxB_inv = tf.matrix_inverse(BTxB_transpose + eye)
        ...
        # y = y * tf.stop_gradient(max_x)
        y.variables = VariableHolder(basis=basis)
        return y


@layer_register()
def Conv2DQuant(x, out_channel, kernel_shape, scale, padding='SAME', stride=1,
                W_init=None, b_init=None, nl=tf.identity, split=1, use_bias=True,
    ...
    * ``b``: bias
    ...
    if depth_wise:
        filter_shape = kernel_shape + [in_channel // split, 1]
    else:
    ...
    if split == 1:
        if depth_wise:
            conv = tf.nn.depthwise_conv2d(x, quantized_weight, stride, padding, data_format=data_format)
        else:
    ...
    ret.variables = VariableHolder(W=kernel_in)
    if use_bias:
        ret.variables.b = b
    if fc:
